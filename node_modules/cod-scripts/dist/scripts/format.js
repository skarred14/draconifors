...
const filesToApply = parsedArgs._.length ? [] : ['**/*.+(md|json|yml|yaml|css|less|scss|sass|graphql|ts|tsx)']; // run prettier on all non-js files

const prettierResult = spawn.sync(resolveBin('prettier'), [...config, ...ignore, ...write, ...filesToApply].concat(relativeArgs), {
  stdio: 'inherit'
});
